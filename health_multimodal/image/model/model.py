...
def get_biovil_resnet(pretrained) -> ImageModel:
    ...
    image_model = ImageModel(
        ...
        joint_feature_size=JOINT_FEATURE_SIZE,  # 128
        pretrained_model_path=pretrained,  # dalla mia path... todo check
    )
    return image_model
...
class ImageModel(nn.Module):
    ...
    def __init__(self,
        ...
        # print("hidden_dim", joint_feature_size)
        print("joint_feature_size", joint_feature_size)
    ...
    def forward(self, x: torch.Tensor):  # -> Tensor:  # ImageModelOutput:
        ...
        # todo xxx remove MODIFICA ENORME
        # print("image MODEL forward and classifier:", self.classifier)
        # logits = self.classifier(pooled_x) if self.classifier else None XXX UNCOMMMENT
        ...
        return projected_global_embedding
...
class ImageEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_patch_embeddings: bool = False) -> TypeImageEncoder:
        ...
        # print("solo encoder forward") xxx ok!
        x = self.encoder(x)
