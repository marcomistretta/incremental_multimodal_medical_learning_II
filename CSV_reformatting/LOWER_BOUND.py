...
#         ll = "mcs-" + str_basic + "-prompt"
#         name_summary = str(
#             "runs-ult/"+chex_str + str(dataset) + "/" + str(split) + "/" + str(img_size) + "-" + str(ll))
...
#         for i in range(5):
#             writer.add_scalar("Class Accuracy", accuracy_score(y_true[:, i], y_pred[:, i]), i)
#             writer.add_scalar("Class Precision", precision_score(y_true[:, i], y_pred[:, i], average="weighted"), i)
#             writer.add_scalar("Class Recall", recall_score(y_true[:, i], y_pred[:, i], average="weighted"), i)
#
