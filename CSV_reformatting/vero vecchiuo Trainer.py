...
# SHARED = False  # True,  False #  shared true mette gli altri due a true <3
...
#             train_loader = filter_dataloader_multiclass(train_loader)
...
#         return writer, class_names, train_loader, val_loader, test_loader, prompts
#
#     #  works for normal and for data-incremental
...
#                 if MAX_EMB:
#                     pos_similarities, inds = torch.max(pos_similarities, dim=1)
#                     neg_similarities, inds = torch.max(neg_similarities, dim=1)
...
#
#     #  works for class-incremental one class and both class
...
#                 neg_similarities, inds = torch.max(neg_similarities, dim=1)
#
#             logits = pos_similarities - neg_similarities
#
...
#                 #  NON E' DERIVABILE LOL Take the maximum similarity as the predicted label
#                 #  predicted_labels[:, i] = torch.argmax(torch.cat([neg_similarities, pos_similarities], dim=1), dim=1)
#                 logits[:, i] = pos_similarities - neg_similarities  #  grandissima differnza
#
...
#                     if PRED_LOGIT_DIFF:
#                         tmp_score[:, i] = (pos_similarities - neg_similarities + 2) / 4  # 
#
#                         # Calculate the similarities between the image and the positive and negative prompts
#                         logits[:, i] = pos_similarities - neg_similarities
#
...
#                     tmp = labels
#                     labels = change_values(labels)
#                 # loss = criterion(loss_predicted_labels, loss_labels)  #  occhio alla differenza loss_ non loss_
#                 # loss = criterion(logits, labels)  #  occhio alla differenza loss_ non loss_
#                 loss = criterion(logits, labels)
#
...
#             self.plot_pca_and_tsne_text_embeddings(epoch, epochs)
#         else:
#             self.plot_cosine_similarity_text_embs_only_pos_prompts(epoch, epochs)
#
#     @staticmethod  #  for class-incremental one class with intersection
...
#         return dataloaders
#
#     @staticmethod  #  for data-incremental and class-incremental two class no intersection
...
#
#     @torch.no_grad()
#     def plot_pca_and_tsne_text_embeddings(self, epoch, epochs):
...
#     #  check se funziona sia per 2d che per 1d
